  ...
  if (WSREP(thd))
  {
    ...
    */
    if (lex->sql_command== SQLCOM_LOCK_TABLES && wsrep_convert_LOCK_to_trx)
    {
      ...
      {
	if (table->lock_type >= TL_WRITE_MIN)
        {
  ...
  */
       ...
       tables= tables->next_global, i++)
  {
    ...
    if (table_ref->sequence)
    {
      ...
      */
      want_access= ((table_ref->lock_type > TL_WRITE_MIN) ?
					...
					bool skip_locked)
{
       ...
       tables= tables->next_local)
  {
    ...
    tables->mdl_request.set_type((lock_type >= TL_WRITE_MIN) ?
...
bool multi_delete_set_locks_and_link_aux_tables(LEX *lex)
{
       ...
       target_tbl; target_tbl= target_tbl->next_local)
  {
    ...
    DBUG_ASSERT(walk->lock_type >= TL_WRITE_MIN);
