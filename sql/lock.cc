  ...
  for (i=0 ; i<count; i++)
  {
    ...
    */
        ...
        ! log_table_write_query)
    {
      ...
      */
      if ((t->reginfo.lock_type >= TL_READ_MAX)
    ...
    if (t->reginfo.lock_type >= TL_WRITE_MIN)
    {
    ...
    */
    DBUG_ASSERT(t->s->tmp_table ||
                thd->mdl_context.is_lock_owner(MDL_key::TABLE,
                                 ...
                                 t->reginfo.lock_type >= TL_WRITE_MIN ?
...
static int lock_external(THD *thd, TABLE **tables, uint count)
{
  ...
  for (i=1 ; i <= count ; i++, tables++)
  {
	...
	((*tables)->reginfo.lock_type >= TL_READ &&
	 (*tables)->reginfo.lock_type <= TL_READ_MAX))
      lock_type=F_RDLCK;
...
int mysql_unlock_read_tables(THD *thd, MYSQL_LOCK *sql_lock)
{
  ...
  for (i=found=0 ; i < sql_lock->table_count ; i++)
  {
    ...
    if ((uint) sql_lock->table[i]->reginfo.lock_type >= TL_WRITE_MIN)
    {
  ...
  {
    if (sql_lock->locks[i]->type >= TL_WRITE_MIN)
    {
