	...
	const trx_t*  trx1)
{
	...
	const trx_state_t trx1_state= trx1->state;
	trx_mutex_enter(lock_rec2->trx);
	const trx_state_t trx2_state= lock_rec2->trx->state;
	trx_mutex_exit(lock_rec2->trx);

	ut_ad(trx1_state == TRX_STATE_ACTIVE);

	ut_ad(trx2_state == TRX_STATE_ACTIVE ||
	      trx2_state == TRX_STATE_COMMITTED_IN_MEMORY ||
	      trx2_state == TRX_STATE_PREPARED);
	...
	we can keep trx1 waiting for the lock. */
	if (wsrep_trx_order_before(lock_rec2->trx->mysql_thd, trx1->mysql_thd))
		return;
	...
	WSREP_INFO("Trx state trx_id: " TRX_ID_FMT " state: %d"
		   " trx_id2: " TRX_ID_FMT " state: %d",
		   trx1->id, trx1_state,
		   lock_rec2->trx->id, trx2_state);
	mtr_t mtr;
				...
				request is really for a 'gap' type lock */
{
	...
	if ((type_mode & LOCK_GAP || lock_rec_get_gap(lock2))
		...
		ut_d(wsrep_assert_no_bf_bf_wait(NULL, lock2, trx));
#endif /* WITH_WSREP */
